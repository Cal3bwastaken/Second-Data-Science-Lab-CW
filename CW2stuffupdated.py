...
def roc_func(df, country, first_year, second_year):
    ...
    start and end year
    """
    corr_r = np.diff(df[country].loc[first_year:second_year])
    corr_roc_r = (corr_r / df[country].loc[first_year:second_year][:-1]) * 100
    mean = corr_roc_r.mean()
    return mean
...
plt.title('GDP and CO2 Emissions Globally (1976 - 2024)')
...
# Axis 1 GDP
ax1.plot(GDP_T['United Arab Emirates'], label='UAE (GDP)', color='blue',
         linestyle='-')
ax1.plot(GDP_T['China'], label='China (GDP)', color='blue', linestyle='--')
ax1.plot(GDP_T['Bolivia'], label='Bolivia (GDP)', color='blue', linestyle='-.')
ax1.plot(GDP_T['Uganda'], label='Uganda (GDP)', color='blue', linestyle=':')
...
ax2.plot(CO2_T['United Arab Emirates'], label='UAE (CO2)', color='green',
         linestyle='-')
ax2.plot(CO2_T['China'], label='China (CO2)', color='green', linestyle='--')
ax2.plot(CO2_T['Bolivia'], label='Bolivia (CO2)', color='green',
         linestyle='-.')
ax2.plot(CO2_T['Uganda'], label='Uganda (CO2)', color='green', linestyle=':')
...
plt.title('GDP and CO2 Emissions (1976 - 2024)')
